					...
					<ul class="nav nav-tabs" id="myTab">
						...
						<li class="active">
							<a >
								...
								<?php if((ACTION_NAME) == "provedit"): ?>修改流程<?php else: ?>添加流程<?php endif; ?>
							</a>
						</li>
					</ul>
					<div class="tab-content">
						<div class="tab-pane in active">
							<form  id="ajaxForm" class="form-horizontal" action="<?php echo U('Leaved/update_prov');?></eq>" method="post" enctype="multipart/form-data">
                                                            <input type="hidden" name="f_id" value="<?php echo ($data["f_id"]); ?>" id="user_and_dep"/>	
								...
								<div class="form-group">
										...
										<div class="col-sm-9">
                                                                                            ...
                                                                                            <input  type="checkbox" class="ace" level="action"  checked/>
											<span class="lbl" id="cate_prov"> </span>
								...
								<div class="form-group">
									...
									<div class="col-sm-9" id="prov_view">
										<div class="mod-tree-people__input_child">
											<ul class="token-input-list_prov input_text" onclick="return showSboxCommon(this)" style="padding-top:3px">
												
											</ul>
                                                                                    <input type="hidden" id="f_prov_input_eidt" class="f_prov_input" value="[]" name="f_prov" />
										</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-2">
										<input type="radio" name="f_status" class="ace"
										       <?php if(($data["f_status"]) == "1"): ?>checked<?php else: endif; ?>
												 value="1"/>
										<span class="lbl"> 已发布 </span>
										<input  type="radio" name="f_status" class="ace"
										<?php if(($data["f_status"]) == "2"): ?>checked<?php else: endif; ?> value="2"/>
...
function showSelectArea_view(list){
	var ss = '';
	for(var s in list){
		var s_obj = list[s];
		if(s_obj.t == 'd'){
...
function showSboxCommon(ele){
	cs_list = $.parseJSON($("#f_prov_input_eidt").val());
	sbox_obj.limit = 1;
	sbox_obj.callback = function(cs_list){
		var res = showSelectArea_view(cs_list);
		$(ele).html(res);
		$("#f_prov_input_eidt").val(JSON.stringify(cs_list));
	}
	showSbox();
}

function addProvDeal(){
	var str = '<div class="mod-tree-people__input_child"><ul class="token-input-list_prov input_text" onclick="return showSboxCommon(this)" style="padding-top:3px"><input type="hidden" class="f_prov_input" value="[]" name="f_prov[]" /></ul><span class="prov_del" onclick="removeThisElement(this);">删除</span></div>';
	$("#prov_view").append(str);
}

seajs.use(['$'],function(jq){
	jq(document).ready(function(){
           var ucs_list=[{id:thelist.uname,pic:thelist.f_upic,t:'u'}];
		var res = showSelectArea_view(ucs_list);
		$(".token-input-list_view").html(res);
		$("#cate_prov").html(thelist.ctname);
                var scs_list=[{id:thelist.suname,pic:thelist.f_spic,t:'u'}];
                $("#f_prov_input_eidt").val(JSON.stringify(scs_list));
                var sres = showSelectArea_view(scs_list);
                console.log(sres);
                $(".token-input-list_prov").html(sres);
	});
});
