				...
				<div class="tabbable">
					...
					<div class="tab-content col-xs-10">
						...
						<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
							<td class="center"><?php echo ($v["f_id"]); ?></td>
							<td><?php echo ($v["f_title"]); ?></td>
							
							<td class="col-xs-3">
								<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
									<button class="btn btn-xs btn-inverse" title="添加子部门" id="addChild"
											data-url="<?php echo U('Departmant/add?f_id='.$v['f_id']);?>">
										<i class="icon-bookmark-empty bigger-120"></i>
										添加子部门
									</button>
									<button class="btn btn-xs btn-info" title="编辑" id="edit" data-url="<?php echo U('Departmant/edit?f_id='.$v['f_id']);?>">
										<i class="icon-edit bigger-120"></i>
										编辑
									</button>
									<button class="btn btn-xs btn-danger" title="删除" id="del" data-id="<?php echo ($v["f_id"]); ?>">
	...
	seajs.use(['$','bootbox'],function($){
		...
		$("button#del").click(function(){
			if(confirm("确定要删除该部门吗？")){
				...
				$.ajax({
					url:"<?php echo U('Departmant/delete');?>",
					data:{id:$id},
					type:"get",
					success:function(data){
						if(data.status > 0 ){
