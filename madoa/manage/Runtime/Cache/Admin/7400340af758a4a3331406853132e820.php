				...
				<div class="tabbable">
					...
					<div class="tab-content col-xs-12">
						...
						<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
							...
							<td>
								<?php if(($v["status"]) == "1"): ?><span class='label label-sm label-success arrowed arrowed-righ'>
										启用
									</span>
									<?php else: ?>
							...
							<td class="col-xs-2">
								<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
									<button class="btn btn-xs btn-info edit"  data-url="<?php echo U('Node/edit?id='.$v['id']);?>">
										<i class="icon-edit bigger-120"></i>
										编辑
									</button>
									<button class="btn btn-xs btn-danger del"  data-id="<?php echo ($v["id"]); ?>">
							...
							<?php if(!empty($v["children"])): if(is_array($v["children"])): foreach($v["children"] as $key=>$j): ?><tr>
										<td ><?php echo str_repeat("&nbsp;",$j['level']*2);?>—| 控制器</td>
										<td> <?php echo str_repeat("&nbsp;",$j['level']*2);?>—| <?php echo ($j["name"]); ?></td>
										<td> <?php echo str_repeat("&nbsp;",$j['level']*2);?>—| <?php echo ($j["title"]); ?></td>
										<td>
											<?php if(($j["status"]) == "1"): ?><span class='label label-sm label-success arrowed arrowed-righ'>
										启用
									</span>
												<?php else: ?>
									<span class='label label-sm label-Default arrowed arrowed-righ'>
										...
										<td class="col-xs-2">
											<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
												<button class="btn btn-xs btn-info edit"   data-url="<?php echo U('Node/edit?id='.$j['id']);?>">
													<i class="icon-edit bigger-120"></i>
													编辑
												</button>
												<button class="btn btn-xs btn-danger del"   data-id="<?php echo ($j["id"]); ?>">
													<i class="icon-trash bigger-120"></i>
													删除
												</button>
											</div>
									...
									<?php if(!empty($j["children"])): if(is_array($j["children"])): foreach($j["children"] as $key=>$x): ?><tr>
												<td ><?php echo str_repeat("&nbsp;",$x['level']*3);?>—| 方法</td>
												<td> <?php echo str_repeat("&nbsp;",$x['level']*3);?>—| <?php echo ($x["name"]); ?></td>
												<td> <?php echo str_repeat("&nbsp;",$x['level']*3);?>—| <?php echo ($x["title"]); ?></td>
												<td>
													<?php if(($x["status"]) == "1"): ?><span class='label label-sm label-success arrowed arrowed-righ'>
										启用
									</span>
														<?php else: ?>
									<span class='label label-sm label-Default arrowed arrowed-righ'>
										禁用
												...
												<td class="col-xs-2">
													<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
														<button class="btn btn-xs btn-info edit"  data-url="<?php echo U('Node/edit?id='.$x['id']);?>">
															<i class="icon-edit bigger-120"></i>
															编辑
														</button>
														<button class="btn btn-xs btn-danger del"  data-id="<?php echo ($x["id"]); ?>">
															<i class="icon-trash bigger-120"></i>
															删除
														</button>
													</div>
