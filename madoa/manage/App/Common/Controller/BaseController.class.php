...
class BaseController extends Controller {
	...
	public function  _initialize() {
		...
		$refer = I('SERVER.HTTP_HOST') . I('SERVER.REQUEST_URI');//$_SERVER['REQUEST_URI'];
	...
	public function uploadLogo() {
		...
		$config = array(
			...
			'autoSub'   => false,
			// 图片文件保存目录
			'rootPath'  => C('UPLOAD_PATH') . 'Picture/logo_tmp/',
		);
		...
		if ($info) {
			...
			$image->open(C("UPLOAD_PATH") . '/Picture/logo_tmp/' . $info['savename']);
			$image->thumb(500, 500)->save(C("UPLOAD_PATH") . "/Picture/logo_tmp/" . $thumb);
			//删除原图
			unlink(C("UPLOAD_PATH") . "/Picture/logo_tmp/" . $info["savename"]);
			$info['path']     = C('IMG_URL') . 'logo_tmp/' . $thumb;
	...
	public function  saveLogo() {
		$image = new \Think\Image();
		$image->open(C("UPLOAD_PATH") . '/Picture/logo_tmp/' . I('origin_image_name'));
		...
		if ($res) {
			//删除原图
			unlink(C("UPLOAD_PATH") . '/Picture/logo_tmp/' . I('origin_image_name'));
			//获取原Logo路径
