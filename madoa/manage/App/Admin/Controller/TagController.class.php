...
class TagController extends CommonController {
	...
	public function index() {
		...
		$list = D('tag')->where("f_corpid = '".$f_corpid."'")->order("concat(path,id)")->select();
                //var_dump($list);exit;
		foreach ($list as $k => $v) {
			if ($v['pid'] > 0) {
	...
	public function add() {
                $f_corpid = session('admin.f_corpid');
		$list = D('tag')->where("f_corpid = '".$f_corpid."'")->field("id,pid,path,tagname")->order("concat(path,id)")->select();
		foreach ($list as $k => $v) {
			if ($v['pid'] > 0) {
				$num              = substr_count($v["path"], ",") - 1;
	...
	public function edit() {
		...
		$list[0]             = D('tag')->where("f_corpid = '".$f_corpid."'")->find($item['pid']);
