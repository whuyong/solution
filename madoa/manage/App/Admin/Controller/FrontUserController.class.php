...
class FrontUserController extends CommonController {
	...
	public function  lists() {
		...
		$field          = array("f_id", "f_name", "f_userid", "f_mobile", "f_gender","f_weixinid","f_email","f_department"); //字段
		$order          = I('iSortCol_0') != null ? $field[intval(I('iSortCol_0'))] . " " . I("sSortDir_0") : "id asc";//排序
		$where          = I('sSearch') != "" ? "f_name like '%" . I('sSearch') . "%'" : ""; //搜索用户名
	...
	public function  add() {
		//获取所有角色列表
		$role = M("front_role")->field("id,remark")->select();
	...
	public function edit() {
		//获取用户昵称与所属角色
		$alias   = D("front_user")->where("userid=" . I("uid", 0))->getField("alias");
		$role_id = D("front_role_user")->where("user_id=" . I("uid", 0))->getField("role_id");
		//获取所有角色列表
		$role = M("front_role")->field("id,remark")->select();
		$this->assign("role", $role);
